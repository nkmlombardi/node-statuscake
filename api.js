...
var API_URL = 'https://app.statuscake.com/API'
var AUTH = {}
        ...
        req.end = function(done) {
            Object.getPrototypeOf(req).end.call(req, function(err, res) {
                ...
                if (res.body instanceof Array) {
                    done(err, res.body.map(function(item) {
                        return _.transform(item, function(result, val, key) {
                            result[key.toLowerCase()] = val
                        })
                    }))
                } else {
                    done(err, _.transform(res.body, function(result, val, key) {
                        result[key.toLowerCase()] = val
                    }))
                }
            })
